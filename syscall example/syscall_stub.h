...
// im not exactly proud of this implementation but its good enough <3

using syscall_stub_t = NTSTATUS(NTAPI*)(...);
namespace utils {
#ifdef _KERNEL_MODE
	PVOID get_routine(PCWSTR name) {
		UNICODE_STRING uniName;
		RtlInitUnicodeString(&uniName, name);
		return MmGetSystemRoutineAddress(&uniName);
	}
#endif
...
#else
		auto fn = get_routine(syscalls::get<number>())); // untested but should work for valid syscalls.
#endif
